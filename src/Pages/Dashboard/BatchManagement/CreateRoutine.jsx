...
import useAxiosPublic from "../../../hooks/useAxiosPublic"; // Import custom hook for secure axios instance
...
const CreateRoutine = ({ batchId, closeModal }) => {
    ...
    const axiosSecure = useAxiosPublic(); // Get the axios instance with secure headers
  
    // Fetch batch data
    useEffect(() => {
      const fetchBatchData = async () => {
        try {
          const response = await useAxiosPublic.get(`/batches/${batchId}`);
    ...
    return (
      <form className="text-black" onSubmit={handleSubmit}>
        <h3 className="text-center font-semibold text-black text-xl">
          Create Routine for Batch: <br /> {batchName || "Loading..."}
        </h3>
